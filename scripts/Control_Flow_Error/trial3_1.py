...
#
# This implementation is very similar to TRIAL3 except we remove redundant loading of values that are
# used for comparison. This will hopefully decrease the memory usage and increase execution speed
#
    ...
    with fileinput.FileInput(i_file, inplace=True, backup='.bak') as file:
        for line in file:
            if line.startswith("\tbne\t"):
                ...
                inst = components[0]
                operands = components[1].split(',')
                if operands[0] == operands[1]:
                    print(line.replace(operands[0], "s11", 1), end='')
                    continue
            print(line, end='')
